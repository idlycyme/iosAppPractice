...
- (void)viewDidLoad {
    ...
    /*
    NSString *possibleImage = [self getPossibleEmbededImageURL:self.tweet.text];
    if (possibleImage != nil) {
        NSLog(@"get image %@", possibleImage);
    [self.attachedImage setImageWithURLRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:possibleImage]] placeholderImage:[UIImage imageNamed:@"placeholder"] success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
        self.attachedImage.alpha = 0.9;
        self.attachedImage.image = image;
        // fade in effect
        [UIView beginAnimations:@"fadeIn" context:nil];
        [UIView setAnimationDuration:5.0];
        self.attachedImage.alpha = 1.0;
        [UIView commitAnimations];
        self.attachedImage.image = image;
    } failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error) {
        // network error
        NSLog(@"[ERROR Failed with attached image retrieval: %@", error);
    }];
    }*/
